...
// Desktop-only sidebar rows. The mobile tab bar is a fixed five-column grid
// (two tabs, the speed dial, two tabs), so anything beyond NAV can only live in
// the sidebar — where Pinterest SEO previously had no entry at all despite
// being a top-level feature on both the dashboard and the speed dial.
const SIDEBAR_EXTRA = [{ to: "/boost", label: "Pinterest SEO", icon: Rocket }] as const;
    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="flex">
        ...
        <aside className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col border-r border-sidebar-border bg-sidebar p-4 md:flex">
          ...
          {/* The "Pro tip" panel that sat here quoted an invented conversion
              statistic and pointed at a feature ("the AI describer") that isn't
              in the product. Deleted rather than reworded — a sidebar doesn't
              need advice, and a made-up number is worse than silence. */}
          <button
            onClick={signOut}
...
// New collection has no route of its own — it's a dialog on the storefront —
// so its action deep-links with ?new=1, which that page reads to open the
// dialog on arrival.
const SPEED_ACTIONS = [
  { to: "/pins/attach", label: "Attach product", icon: Link2, x: -114, y: -144 },
  { to: "/pins/create", label: "Create pin", icon: Plus, x: -42, y: -216 },
  {
    to: "/storefront",
    search: { new: 1 },
  ...
  { to: "/boost", label: "Pinterest SEO", icon: Rocket, x: 114, y: -144 },
] as const;
...
function ProfileSwitcher({
  ...
  const [open, setOpen] = useState(false);
  const ref = useRef<HTMLDivElement | null>(null);
  const { all, activeId, setActive } = useLocalPinAccounts(handle);
    ...
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute inset-x-0 top-[calc(100%+8px)] z-50 overflow-hidden rounded-2xl border border-border bg-surface p-2 shadow-elevate">
          ...
          {/* Adding an account used to fire a raw window.prompt() — an
              unstyled, unvalidated browser dialog that couldn't say "that one's
              already added" and looked nothing like the app. The Switch profile
              screen already has the real form, so this points at it. */}
          <Link
            to="/switch-profile"
            onClick={() => setOpen(false)}
